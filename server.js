...
app.post('/api/eleves/importer', requireAuth, async (req, res) => {
    ...
    for (let i = 0; i < eleves.length; i++) {
        const e = eleves[i];
        try {
            const matricule = (e.matricule || '').toUpperCase();
            if (!matricule) {
                echoues++;
            ...
            `, [
                matricule, e.nom, e.prenoms, e.sexe, e.date_naissance || null,
            ...
            // Toute erreur (doublon, date invalide, valeur trop longue, etc.)
            // est comptée et journalisée ici, SANS jamais interrompre la boucle :
            // une seule ligne mauvaise ne doit pas faire échouer tout l'import.
            if (err.code === '23505') {
                existants++;
            } else {
                echoues++;
                erreurs.push({
                    ligne: i + 2,
                    matricule: e.matricule || '(vide)',
                    raison: err.message
                });
                console.error('❌ Import échoué ligne ' + (i + 2) + ' (' + (e.matricule || '?') + '):', err.message);
            }
        }
    }
    ...
    res.json({
        ...
        existants,
        echoues,
        erreurs: erreurs.slice(0, 50), // on limite la réponse pour ne pas l'alourdir
