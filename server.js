...
// GET - Effectif d'une classe (nombre d'élèves inscrits dedans).
// Utilisé par la page d'inscription pour savoir si la classe proposée
// automatiquement est pleine avant de la suggérer au parent.
// max est fixé à 80 par défaut (ajustable si besoin).
app.get('/api/classes/:code/effectif', async (req, res) => {
    const code = req.params.code;
    const MAX_PAR_CLASSE = 80;
    try {
        const result = await pool.query(
            'SELECT COUNT(*) AS effectif FROM eleves WHERE classe = $1',
            [code]
        );
        res.json({
            code,
            effectif: parseInt(result.rows[0].effectif, 10),
            max: MAX_PAR_CLASSE
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
